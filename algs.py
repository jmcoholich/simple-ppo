...
class PPO:
    def __init__(
        ...
        if opt_alg == "adam":
            ...
            raise ValueError(
                "'%s' is not currently a support optimization algorithm" % opt_alg
            )
    ...
    def update(self, wandb=None):
        ...
        with torch.no_grad():
            ...
            self.replay_buffer.compute_gae(
                old_predicted_values, self.gamma, self.gae_lambda
            )  # checked
            # below is fine, just don't use supp
            value_targets = self.replay_buffer.advantages + old_predicted_values[:-1]
            old_log_probs, _ = self.policy.get_log_probs(
                self.replay_buffer.obs[:-1],
                self.replay_buffer.actions[:-1],
                compute_entropy=False,
            )  # again just don't use supp
            ...
            if N in bs_idcs:
                ...
                bs_idcs = bs_idcs[
                    :-1, :
                ]  # get rid of that last value, since it's already gone
            mean_ = (
                self.replay_buffer.advantages.sum()
                - self.replay_buffer.advantages[bs_idcs].sum()
            ) / (N - len(bs_idcs))
        ...
        for _ in range(self.epochs):
            ...
            ppo_clip_obj, entropy = self._compute_ppo_clip_obj(
                old_log_probs, advantages, bs_idcs, compute_entropy=True
            )  # (0.0 != self.entropy_coef)) # Always compute entropy just for logging
            value_loss = self._compute_value_loss(
                value_targets, old_predicted_values[:-1], bs_idcs
            )
            loss = (
                -ppo_clip_obj
    ...
    def _compute_ppo_clip_obj(
        self, old_log_probs, advantages, bs_idcs, compute_entropy=True
    ):
        """Combine with entropy calculation to avoid separate fwd pass for entropy"""
        log_probs, entropy = self.policy.get_log_probs(
            self.replay_buffer.obs[:-1],
            self.replay_buffer.actions[:-1],
        ...
        entropy_mean = (entropy.sum() - entropy[bs_idcs].sum()) / (
            len(entropy) - len(bs_idcs)
        )
        return (
            torch.min(
                ratios * advantages,
    ...
    def _compute_value_loss(self, value_targets, old_predicted_values, bs_idcs):
        ...
        current_value_predictions = self.value_net(
            self.replay_buffer.obs[:-1]
        ).squeeze()
        value_loss = (current_value_predictions - value_targets).pow(2)
        value_pred_clipped = old_predicted_values + torch.clamp(
            current_value_predictions - old_predicted_values, -self.eps, self.eps
        )
